...
const resolveUserTableByRole = (role) => {
  if (['employer_admin', 'employer_staff', 'consultant_admin'].includes(role)) {
...
export const verifyOTP = async (req, res) => {
  ...
  const table = resolveUserTableByRole(role);
...
export const resendOTP = async (req, res) => {
  ...
  try {
    ...
    // ⏳ Prevent OTP spam (60s cooldown)
    if (user.otp_expires_at) {
      const lastOtpTime =
        new Date(user.otp_expires_at).getTime() - 15 * 60 * 1000;
